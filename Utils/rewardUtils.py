...
def get_min_distance_from_edges(bottomPos, bottomYaw, current_bottom_obj_size, 
                                    targetPos, targetYaw, current_target_obj_size):
    '''Calculates the distance between the target's COM and each of the four edges, and returns the minimum among all
        the distances

        +ve distance - outside the border
        -ve distance - inside the border
    '''
    target_cartesian_pos = targetPos.reshape((3, 1))
    bottom_com = bottomPos.reshape((3, 1))
    ...
    # print("Shapes: \nR: {}\ntargetCornersStandard: {}".format(R.shape, targetCornersStandard.shape))
...
def get_max_extent_of_target_from_bottom2(bottomPos, bottomOrn, current_bottom_obj_size, 
    ...
    '''Calculates in cms, the max outward extent of the target object from the bottom object
    '''
